...
def create_peaknet_model(yaml_path, weights_path, gpu_id, compile_model=False, compile_mode='default'):
    """Create PeakNet model for compute simulation, or None for no-op"""

    # Handle no-op case (when yaml_path is None or empty)
    if not yaml_path:
        print("No-op compute mode: no YAML path specified, skipping PeakNet model creation")
        return None, None, None

    # Normal PeakNet creation
    try:
        ...
        print("Falling back to no-op mode")
        return None, None, None
